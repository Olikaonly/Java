...
public class ClientHandler {
    ...
    private void logic() {
        try {
            ...
            System.out.println("Клиент " + username + " отключился"); //сообщение о том что клиент отключился
    ...
    private boolean consumeRegularMessage(String inputMessage) {
        if (inputMessage.startsWith("/")) {
            ...
            if (inputMessage.startsWith("/w ")) {
                String[] tokens = inputMessage.split("\\s+", 3); //запрет на разбивку имени пробелами
                server.sendPersonalMessage(this, tokens[1], tokens[2]);// отправка сообщения
            }
            return true;
        }
    ...
    private boolean consumeAuthorizeMessage(String message) { //метод обработки сообщений авторизации
        if (message.startsWith("/auth ")) { // /auth bob
            ...
            String selectedUsername = tokens[1]; //проверка на занятость имени
            if (server.isUsernameUsed(selectedUsername)) {
                sendMessage("SERVER: Данное имя пользователя уже занято");
                return false;
            }
            username = selectedUsername; //запоминаем имя пользователя
            sendMessage("/authok");
