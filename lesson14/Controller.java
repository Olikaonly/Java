...
public class Controller {
    ...
    public void setAuthorized(boolean authorized) {
        msgPanel.setVisible(authorized); //переключение авторизации
        msgPanel.setManaged(authorized);
        authPanel.setVisible(!authorized);
        authPanel.setManaged(!authorized);
        clientsListView.setVisible(authorized); // после авторизации, видим список пользователей
    ...
    public void sendCloseRequest() { //отключение от сервера, закрытием окна
        try {
            if (out != null) {
                out.writeUTF("/exit");
            }
    ...
    private void logic() { //
        try {
            ...
            while (true) { //отключение от сервера
                String inputMessage = in.readUTF();
                if (inputMessage.startsWith("/")) {
                    ...
                    if (inputMessage.startsWith("/clients_list ")) { //добавление в список пользователей
                        Platform.runLater(() -> { //делаем в Javafx
    ...
    private void closeConnection() { //отключение от сервера
        setAuthorized(false);
        try {
            if (in != null) {
                in.close();
    ...
    public void clientsListDoubleClick(MouseEvent mouseEvent) { //отправка личного сообщения по дабл клик
